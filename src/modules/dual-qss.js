(function(root){
  ...
  function positionHtml(d){
    ...
    return`<section class="panel"><h3>Measurement position ${help('OnePointPattern is a single scheduled measurement, not a spatial heatmap. The outline uses the nominal substrate/target geometry and the dashed line shows EdgeExclusion when available.')}</h3><svg viewBox="0 0 220 205" role="img" aria-label="Single measurement position on nominal circular substrate"><circle cx="${cx}" cy="${cy}" r="${R}" fill="var(--panel2)" stroke="var(--soft)" stroke-width="2"/>${Number.isFinite(ri)?`<circle cx="${cx}" cy="${cy}" r="${ri}" fill="none" stroke="var(--muted)" stroke-width="1.2" stroke-dasharray="5,4"/>`:''}<line x1="${cx-R}" x2="${cx+R}" y1="${cy}" y2="${cy}" stroke="var(--grid2)"/><line x1="${cx}" x2="${cx}" y1="${cy-R}" y2="${cy+R}" stroke="var(--grid2)"/><circle cx="${px}" cy="${py}" r="6" fill="var(--blue)" stroke="var(--text)" stroke-width="1.5"/><text x="${cx}" y="199" text-anchor="middle" fill="var(--muted)" font-size="10">Ø${fmt(g.radius*2,0)} mm${Number.isFinite(d.edgeExclusion)?` · exclusion ${fmt(d.edgeExclusion,1)} mm`:''} · point (${fmt(g.coord.x,1)}, ${fmt(g.coord.y,1)}) mm</text></svg></section>`;
  }
  ...
  PV.modules=PV.modules||{};PV.modules.dualQss={types:['DualQssMeasurement'],parse,analyze,render,parseTransient,classifyRange,pointRows,measurementGeometry};PV.registry.register(PV.modules.dualQss);
