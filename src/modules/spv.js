  ...
  function render(host,data,analysis){
    ...
    function shell(){
      ...
      host.innerHTML=`<div class="module-grid spv-module"><aside class="side">
        <section class="panel"><h3>Measurement ${help('Core SPV XML identity and sample context. Signal-channel settings and validation profiles are separated below.')}</h3><dl class="meta"><dt>Result</dt><dd>${esc(data.resultName)}</dd><dt>Recipe</dt><dd>${esc(data.name)}</dd><dt>Substrate</dt><dd>${esc(data.substrateId)}</dd><dt>Status</dt><dd>${esc(data.status)}</dd><dt>Pattern</dt><dd>${esc(data.patternName||data.patternType)}</dd></dl></section>
        ...
        <details class="panel"><summary>Acquisition / validation</summary><dl class="meta meta-detail"><dt>Calculation profile</dt><dd>${esc(calc)}</dd><dt>Geometry profile</dt><dd>${esc(geom)}</dd><dt>Channels</dt><dd>${fmt(data.wavelength8,0)} / ${fmt(data.wavelength6,0)} nm</dd><dt>Chuck temperature</dt><dd>${fmt(data.chuckTemperature,2)} °C</dd></dl><p class="note meta-detail">Paired PV-2000 output validates the standard two-wavelength profiles and the finite-wafer/back-surface Enhanced N-type profile. Parsed-signal, texture-corrected, manual-linearity and Enhanced P-type branches remain outside the validated envelope.</p></details>
