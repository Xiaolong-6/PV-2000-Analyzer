...
<dl class="meta">${meta('Result',d.resultName)}${meta('Recipe',d.name)}${meta('Substrate',d.substrateId)}${meta('Status',d.status)}${meta('Pattern',pattern())}${meta('Target',target())}${meta('Doping',`${fmt(d.doping)} cm⁻³ ${d.dopingType}`)}</dl>\
        ...
        <details class="panel"><summary>Validation / provenance</summary><dl class="meta meta-detail">${meta('Geometry',geometryText,'Calculation and geometry validation are tracked separately. SquareRegionPattern uses its structured Region + Dimension fields; incomplete terminated schedules are prefix-inferred and not vendor-validated.')}${meta('Calculation',calcText,'Validated Basore J0 requires the complete established two-intensity path. If the second iteration is missing or shorter, available first-iteration quantities remain usable while J0 is unavailable at unpaired sites.')}</dl></details>
        <details class="panel"><summary>Acquisition metadata</summary><dl class="meta meta-detail">${meta('QSS intensities',qssText)}${meta('Wafer thickness',`${fmt(d.waferThickness)} µm`)}${meta('Optical factor',fmt(d.opticalFactor))}${meta('Probe / bias',`${d.probe||'—'} / ${d.bias||'—'}`)}${meta('Result time',d.end)}${meta('Elapsed',d.elapsed)}${meta('Laser power',fmt(d.laserPower))}${meta('uPCD averaging 1 / 2',`${fmt(d.avgMode,0)} / ${fmt(d.secondAvgMode,0)}`)}${meta('Evaluation mode',d.evaluationMode||'—')}${meta('Chuck temperature 1 / 2',`${fmt(d.temperatures[0])} / ${fmt(d.temperatures[1])} °C`)}${meta('Measurement velocity 1 / 2',`${fmt(d.measurementVelocities[0])} / ${fmt(d.measurementVelocities[1])}`)}${meta('Tau steady-state factor 1 / 2',`${fmt(d.tauSteadyStateFactors[0],6)} / ${fmt(d.tauSteadyStateFactors[1],6)}`)}${meta('QDC 1 / 2',`${fmt(d.qdcValues[0],6)} / ${fmt(d.qdcValues[1],6)}`)}${meta('Autosetting',d.autoset||'—')}${meta('Rastering',d.doRastering||'—')}${meta('Save transient',d.saveTransient||'—')}</dl></details>
      </aside>${onePoint?'<div class="single-analysis-workspace">':''}<section class="plots overview"><div class="panel chart"><header><b>${onePoint?'Measurement position':'Wafer map'}</b><span class="grow"></span><select id="jMetric">${options()}</select>${onePoint?'':`<select id="jMapMode"><option value="filled">Filled</option><option value="points">Points</option></select>`}${PV.plot.axisControls('jMapAxes')}<button id="jExportMap">Export</button></header><div class="canvas-wrap"><canvas id="jMap"></canvas></div></div>${onePoint?'':`<div class="panel chart"><header><b>Distribution</b><span class="grow"></span>${PV.plot.axisControls('jHistAxes',{distribution:true,swapped:histSwapped})}${PV.plot.binControls('jHistBins',histBins)}<button id="jExportHist">Export</button></header><div class="canvas-wrap"><canvas id="jHist"></canvas></div></div>`}</section><section class="plots detail"><section class="panel"><h3>${onePoint?'Measurement point':'Selected site'}</h3><div id="jSelected">${selectedHtml()}</div></section></section>${onePoint?'</div>':''}</div>`;
