(function(root){
  ...
  function effectiveHalf(size,edge=0){
    const half=size/2;
  ...
  function parse(parsed){
    ...
    return{...c,values,coords,iterations:iterations.length,patternType,patternName:X.text(pattern,'Name',''),targetType,targetWidth,targetHeight,diameter,edgeExclusion,pitchX,pitchY,mapHalfWidth,mapHalfHeight,mapRadius,qssMilli,
  ...
  function insideTarget(d,x,y,scheduled=true){
    ...
    if(d.targetType==='PseudoSquareCell')return Math.abs(x)<=hw&&Math.abs(y)<=hh&&x*x+y*y<=r*r+1e-9;
    if(d.targetType==='RoundWafer')return x*x+y*y<=r*r+1e-9;
    if(d.targetType==='SquareCell')return Math.abs(x)<=hw&&Math.abs(y)<=hh;
    return true;
  }
  ...
  function render(host,d,a){
    let metricKey='j0',filterKey='j0',histBins=30,histSwapped=true,pointsMode=false,zoom={map:{x:null,y:null},hist:{x:null,y:null}},range=finiteRange(a.metrics.j0.values),filterLo=range.min,filterHi=range.max,mask=validMask(a.metrics.j0.values,filterLo,filterHi);
    ...
    const target=()=>d.targetType==='PseudoSquareCell'?`${fmt(d.targetWidth)} × ${fmt(d.targetHeight)} mm pseudo-square · Ø${fmt(d.diameter)} mm mask · edge ${fmt(d.edgeExclusion)} mm`:d.targetType;
    ...
    function shell(){
      ...
      host.innerHTML=`<div class="module-grid jzero-module"><aside class="side">
        <section class="panel"><h3>Emitter J0 map ${help('JZeroMeasurement combines two QSS-µPCD lifetime maps measured at the first and second QSS intensities. Basore J0 is derived site-by-site from the two small-perturbation lifetimes.')}</h3><dl class="meta">${meta('Result',d.resultName)}${meta('Recipe',d.name)}${meta('Substrate',d.substrateId)}${meta('Status',d.status)}${meta('Pattern',`${d.patternName} · ${fmt(d.pitchX)} × ${fmt(d.pitchY)} mm`)}${meta('Target',target())}${meta('QSS intensities',`${fmt(d.qssMilli[0]/1000,2)} / ${fmt(d.qssMilli[1]/1000,2)} sun`)}${meta('Wafer thickness',`${fmt(d.waferThickness)} µm`)}${meta('Doping',`${fmt(d.doping)} cm⁻³ ${d.dopingType}`)}${meta('Optical factor',fmt(d.opticalFactor))}${meta('Probe / bias',`${d.probe||'—'} / ${d.bias||'—'}`)}</dl></section>
      ...
      </aside><section class="plots"><div class="panel chart"><header><b>Wafer map</b><span class="grow"></span><select id="jMetric">${options()}</select><select id="jMapMode"><option value="filled">Filled</option><option value="points">Points</option></select>${PV.plot.axisControls('jMapAxes')}<button id="jExportMap">Export</button></header><div class="canvas-wrap"><canvas id="jMap"></canvas></div></div><div class="panel chart"><header><b>Distribution</b><span class="grow"></span>${PV.plot.axisControls('jHistAxes',{distribution:true,swapped:histSwapped})}${PV.plot.binControls('jHistBins',histBins)}<button id="jExportHist">Export</button></header><div class="canvas-wrap"><canvas id="jHist"></canvas></div></div></section></div>`;
      host.querySelector('#jMetric').value=metricKey;host.querySelector('#jFilterMetric').value=filterKey;host.querySelector('#jMapMode').value=pointsMode?'points':'filled';
      host.querySelector('#jMetric').onchange=e=>{metricKey=e.target.value;zoom={map:{x:null,y:null},hist:{x:null,y:null}};redraw()};
      host.querySelector('#jMapMode').onchange=e=>{pointsMode=e.target.value==='points';redraw()};
  ...
  PV.modules=PV.modules||{};PV.modules.jzero={types:['JZeroMeasurement'],parse,analyze,render,generation,smax,basoreJ0,impliedVoc,insideTarget,constants:{NI_BASORE_COMPAT,NI_VOC_300}};PV.registry.register(PV.modules.jzero);
