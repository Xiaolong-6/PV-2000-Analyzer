...
${md('Back Surface Shift',d.backSurfaceShift?'True':'False','PV2000 exposes this Boolean adjustment. Its effect was not identified in the supplied tests, so the reverse-engineered method records but does not apply it.')}</dl>\
    ...
    function drawDit(){
      ...
      const ditMeta=host.querySelector('#ditDitMeta'),
        compactFit=analysis.options.pchipEnabled?analysis.mode+' · '+fitLabel:analysis.mode+' · PCHIP off';
      ditMeta.textContent=compactFit;
      ditMeta.title=analysis.options.pchipEnabled
        ?analysis.mode+' · PV2000 min '+sci(s.Dit,2)+' · '+fitLabel+' '+scaleLabel+(coverage?' · '+coverage:' · midgap '+sci(s.MidgapDit,2))
        :analysis.mode+' · PV2000 min '+sci(s.Dit,2)+' · PCHIP off'}
    function drawMap(){
