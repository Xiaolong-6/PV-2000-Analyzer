...
- `QssUpcdMeasurement` — QSS-µPCD map analysis with raw lifetime preservation, sentinel-aware scientific validity, Smax, PV-2000-compatible Implied Voc, optional Physical Si/Ge estimates, configurable lifetime→SRV post-processing, valid-data filtering, map/distribution/acquisition views and CSV export.
- `DualQssMeasurement` — single-point injection-intensity sweep analysis with PV-2000 raw `LifeTime` (`TransientInfo@LifeTime`) as the validated default curve, separate XML `Values` diagnostics, per-injection stored transient inspection, nominal one-point measurement-position context, local LP/HP/repeat overlay and CSV export. The expanded private corpus has 273 exact XML/CSV pairs covering 5833 injection points; vendor result-table `Lifetime`, Δn, Implied Voc and J0 remain a separate unsupported post-processing path until reproduced point-by-point.
