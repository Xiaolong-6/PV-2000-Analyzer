...
- `DualQssMeasurement` — single-point injection-intensity sweep analysis with the raw XML transient-lifetime curve, per-injection stored transient inspection, nominal one-point measurement-position context, local LP/HP/repeat overlay and CSV export. Fifty-seven matching PV-2000 raw CSV exports validate the raw XML intensity/power/lifetime/transient path. The vendor result-table `Lifetime`, Δn, Implied Voc and J0 calculations are distinct post-processing paths and remain intentionally unimplemented until those transformations are reproduced point-by-point.
