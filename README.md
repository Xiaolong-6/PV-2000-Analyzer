...
- `LBICMeasurement` — spatial LBIC raster analysis with dynamic beam/wavelength channels. Paired references validate both the single-beam `SquareRegionPattern` family and the independent multi-beam `MapPattern + PseudoSquareCell` family, including Current / Reflectivity / IQE parity and masked pseudo-square geometry. The default view mirrors Current / Reflectivity / IQE, while raw Direct/Scattered reflection, EQE and unknown channels are under Advanced; calculated diffusion length remains unsupported.
