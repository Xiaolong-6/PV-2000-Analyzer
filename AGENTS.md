...
4. Shared XML/statistics/geometry/theme/export/UI logic belongs in `src/core/`; do not duplicate common HTML escaping, help markup or tooltip helpers in measurement modules. For migrated/new scientific families, keep parsing, quantity provenance/availability, semantic profile metadata, scientific calculation and rendering as explicit layers. Use `PV2000.quantity`, `PV2000.measurement`, `PV2000.profiles` and the normalized geometry envelope rather than inventing module-local equivalents.
