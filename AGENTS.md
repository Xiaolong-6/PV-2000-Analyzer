...
Validation must follow **input/result combinations and units**, not exact fixture constants. The paired LBIC references validate SquareRegionPattern coordinate ordering, total reflectance from direct+scattered percent channels, and the µA-current + FluxCache + reflectance IQE chain (including >=100% reflectance blanking). Do not downgrade the same relationship merely because wavelength, power or FluxCache numeric values differ. Do require new regression when semantics change: different units, different pattern/acquisition ordering, different FluxCache meaning, or genuinely new channel combinations. Multi-beam parsing still needs a real paired multi-wavelength reference. Do not implement a calculated LBIC diffusion-length map from a plausible literature formula; require a multi-wavelength PV-2000 XML plus matching DL output and regression first.
