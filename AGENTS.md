...
5. Preserve the responsive scientific-workspace layout and its semantic column contract. On wide screens, the left sidebar is dataset/analysis state, the middle column is whole-sample/population overview (Map, Distribution and other global views), and the right column is selected-point/local detail (Selected site/pixel/Measurement point plus raw readings, transients, fits, point curves and local profiles). Selected-site/pixel detail must not live in the left sidebar. Multi-point DIT is the explicit layout exception: keep its `Selected site` card directly below the Wafer map in the middle overview column so spatial navigation and the selected-site readout remain one inspection surface; keep Vcpd–Qc, Dit–Vsb, Vsb–Qc and Flatband extraction in the right scientific-detail column. Do not generalize this exception to other analyzers without an explicit review decision. At medium widths (<=1200 CSS px), keep the dataset sidebar and stack the middle/right analysis regions into one content column; narrow/mobile layouts collapse to one column in semantic order. Do not imitate the legacy PV-2000 application. Plot axis-range controls must remain compact header popovers immediately before Export rather than taking permanent chart height. Quantity-driven Map/Distribution/local-profile views should share one active displayed quantity with the Valid-data filter by default; the selection core must still retain cross-metric capability for future explicit advanced use. Dedicated analyzers follow the shared information hierarchy documented in `docs/ARCHITECTURE.md`.
6. Every chart must expose a data export and shared zoom behavior without stealing ordinary pane scrolling: plain wheel/trackpad motion scrolls the current pane; Ctrl/⌘ + wheel in the plot zooms both axes, Ctrl/⌘ + wheel over one axis zooms only that axis, and double-click restores auto scale.
...
PV-2000 QSS XML may encode unavailable lifetime as the numeric controller sentinel `-1 µs`. Preserve that raw XML value for provenance, but keep raw-controller storage separate from final-result semantics: paired PV-2000 exports in the 100-case corpus represent those same sites as lifetime `Ud.`, Smax `0`, and Implied Voc `0`. Scientific analysis must default to treating non-positive lifetime as unavailable before applying the user-controlled validity range. Keep raw XML, vendor-result placeholders, intrinsic availability and user filtering distinct in UI/tooltips/exports, and prevent unavailable sites from setting scientific histogram/map ranges or being smoothed across the sample. A raw inspection mode may expose the XML/controller sentinel but must not label `-1` or its mathematical negative Smax as the vendor result.

The QSS XML family does not provide a trustworthy semiconductor-material identifier. Never infer Si/Ge from filenames, result names or substrate IDs. The default simple-map Implied-Voc path is the vendor-validated `QSS-CALC-IMPLIED-VOC-002` compatibility result and must preserve the recovered DLL constants, temperature convention, zero-intensity `Ud.` rule and non-positive-lifetime numeric placeholder. Physical Si/Ge estimates must require explicit user selection and remain labelled analyzer-side estimates unless matching material-specific PV-2000 output validates them.
