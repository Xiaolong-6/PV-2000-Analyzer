...
Observed XML contains both `TransientInfo@LifeTime` and a `Values` vector. They are not interchangeable.

The paired raw CSV `LifeTime` column matches `TransientInfo@LifeTime`, so the analyzer uses that as canonical raw lifetime and falls back to `Values` only when needed. The validated scalar-result path uses `Values` according to its own profile rules.

A vendor result-table lifetime found in broader exports is another post-processed quantity and is not silently substituted for either XML representation.
...
`QSS-INJ-RESULT-001` validates the current non-Auger Back/Back scalar path on real matching XML + numeric result exports, including teff.d, teff.SS, teff.SS Max, Δn, both Smax values, implied Voc, Basore J0 and K-S J0 where applicable.

Auger correction, alternate source selections, conflicting historical FixedPoints/PseudoSquare result behavior and other categorical branches remain outside this envelope.
