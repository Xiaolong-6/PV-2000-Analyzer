...
In wide three-column layouts, the analyzer shell owns the viewport: toolbar at the top, project footer in the bottom shell row, and the left dataset pane, middle overview pane and right local-detail pane each own their vertical scroll. There is no second page/body scrollbar. The three wide columns use literal equal grid tracks. At medium widths the overview/detail regions return to normal document flow when they stack into one content column. Plot axis controls remain compact header controls immediately before Export. Opening Axes or Bins reveals a full-width settings row in normal chart-header flow: its width follows the chart container, its height follows its content, and only one settings row stays open at a time. The settings row must not use viewport-relative width, absolute positioning or fixed padding reservations to avoid plot overlap. Plain wheel/trackpad motion scrolls the pane; Ctrl/⌘ + wheel performs shared plot zoom, and double-click restores Auto.
...
**DIT multi-point exception:** `Selected site` is placed immediately below the Wafer map in the middle overview column. In DIT, the selector/readout is treated as the direct inspection companion to the spatial map, while Vcpd–Qc, Dit–Vsb, Vsb–Qc and Flatband extraction remain in the right scientific-detail column. This is a DIT-specific product decision and does not redefine the default contract for other analyzers.
...
Single-point datasets preserve the same semantic information hierarchy but switch the wide workspace to **sidebar + one 2fr analysis workspace**. That analysis workspace is itself a two-column grid: position/geometry and selected-point details share the first row, while scientific plots fill subsequent cells. Plot-rich families can span a meaningful chart across both analysis columns; DIT uses two local curves side-by-side and the third across a full row. This avoids both the empty third-column problem and an unnecessarily long single right-hand stack. Multi-point analyzers retain the literal equal-three-column workspace. Measurement-position surfaces remain compact, and this is presentation-only: target geometry, coordinates, selection and exported values are unchanged.
...
User filtering is always separate from intrinsic support/availability. A filtered finite value is not the same state as an unavailable/undefined value. Selected-site/pixel/measurement-point cards therefore identify the **filter quantity** beside a shared **VALID / FILTERED / UNAVAILABLE** badge and a short explanation; algorithm-invalid sites use a distinct invalid state. This prevents an unavailable filter quantity from being misread as meaning that every local result is unavailable. Non-filter families should describe actual per-site quantity availability rather than presenting a blanket success state.
