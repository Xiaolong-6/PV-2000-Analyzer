...
In wide three-column layouts, the analyzer shell owns the viewport: toolbar at the top, project footer in the bottom shell row, and the left dataset pane, middle overview pane and right local-detail pane each own their vertical scroll. There is no second page/body scrollbar. The three wide columns use literal equal grid tracks. At medium widths the overview/detail regions return to normal document flow when they stack into one content column. Plot axis controls remain compact header popovers immediately before Export. Opening Axes or Bins reserves temporary vertical space inside the chart header instead of covering the scientific plot; only one header popover stays open at a time. Plain wheel/trackpad motion scrolls the pane; Ctrl/⌘ + wheel performs shared plot zoom, and double-click restores Auto.
...
User filtering is always separate from intrinsic support/availability. A filtered finite value is not the same state as an unavailable/undefined value. Selected-site/pixel/measurement-point cards therefore keep state explicit: filter-aware families show a shared **VALID / FILTERED / UNAVAILABLE** badge (with algorithm-invalid states remaining distinct where applicable), while non-filter families may use **AVAILABLE / UNAVAILABLE** when that is the scientifically honest distinction.
