...
Single-point datasets preserve the same semantic information hierarchy but switch the wide workspace to **sidebar + one 2fr analysis column**. The compact position/geometry view sits above selected-point/detail content instead of reserving two mostly empty right-hand columns. Multi-point analyzers retain the literal equal-three-column workspace. Measurement-position surfaces remain compact, and this is presentation-only: target geometry, coordinates, selection and exported values are unchanged.
