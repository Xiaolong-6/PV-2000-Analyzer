...
In wide three-column layouts, the analyzer shell owns the viewport: toolbar at the top, project footer in the bottom shell row, and the left dataset pane, middle overview pane and right local-detail pane each own their vertical scroll. There is no second page/body scrollbar. The three wide columns use literal equal grid tracks. At medium widths the overview/detail regions return to normal document flow when they stack into one content column. Plot axis controls remain compact header popovers immediately before Export. Plain wheel/trackpad motion scrolls the pane; Ctrl/⌘ + wheel performs shared plot zoom, and double-click restores Auto.
