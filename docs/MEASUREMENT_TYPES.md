...
3. Extend LBIC only when a categorical input/output path changes beyond the recorded current-enabled single-beam, current-enabled multi-beam pseudo-square, and reflectance-only SquareRegion families; add diffusion length only after the vendor DL algorithm is established from matching real output.
4. EOT.
5. IV.
